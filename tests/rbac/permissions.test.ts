...
describe("permission catalog (RBAC matrix §10)", () => {
  ...
  it("never grants parents team-wide evaluation/ranking views (§6.18, §12.3)", () => {
    expect(roleHasPermission("PARENT", "evaluations.view_team")).toBe(false);
    expect(roleHasPermission("PARENT", "evaluations.manage_templates")).toBe(false);
  });
  ...
  it("restricts audit log access to admins (§6.23)", () => {
    ...
    expect(roleHasPermission("PARENT", "audit.view")).toBe(false);
  });

  it("scopes coach capabilities to TEAM and parent capabilities to CHILD", () => {
    expect(grantedScope("COACH", "attendance.record")).toBe("TEAM");
    expect(grantedScope("COACH", "roster.view_full")).toBe("TEAM");
    expect(grantedScope("PARENT", "attendance.view_own_child")).toBe("CHILD");
    expect(grantedScope("PARENT", "roster.view_safe")).toBe("CHILD");
  });

  it("only lets admins manage clubs/teams (§6.2, §6.4)", () => {
    expect(roleHasPermission("COACH", "teams.manage")).toBe(false);
    expect(roleHasPermission("PARENT", "teams.manage")).toBe(false);

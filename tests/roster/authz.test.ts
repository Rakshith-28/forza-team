...
import { playerAccountUpdatePlayerSchema } from "@/modules/roster/schemas";
...
// A player account with TWO linked children on two different teams.
...
// ---------------------------------------------------------------------------
// 1 — Player cannot edit another child's profile
// ---------------------------------------------------------------------------
describe("player cannot edit a child they aren't linked to", () => {
  it("rejects updateOwnChild for a non-linked player (before any DB access)", async () => {
    await expect(
      updateOwnChild(playerMulti, "someone-elses-kid", { preferredName: "x" } as never),
    ).rejects.toBeInstanceOf(ForbiddenError);
  });

  it("permits child scope only for the player's own linked children", () => {
    expect(can(playerMulti, "players.edit_limited_own_child", { clubId: CLUB_A, playerId: "kid-1" })).toBe(true);
    expect(can(playerMulti, "players.edit_limited_own_child", { clubId: CLUB_A, playerId: "kid-2" })).toBe(true);
    expect(can(playerMulti, "players.edit_limited_own_child", { clubId: CLUB_A, playerId: "kid-other" })).toBe(false);
  });
});

// ---------------------------------------------------------------------------
// 2 + 8 — Player-safe roster strips restricted fields; photo gated by setting
// ---------------------------------------------------------------------------
describe("player-safe roster projection", () => {
  ...
  it("hides photos when show_player_photos_to_players is false", () => {
    expect(playerSafePlayer(full, { showPhotos: false }).photoUrl).toBeNull();
    expect(playerSafePlayer(full).photoUrl).toBeNull(); // default = hidden
    expect(playerSafePlayer(full, { showPhotos: true }).photoUrl).toBe("https://cdn/p1.jpg");
  });
});
...
// ---------------------------------------------------------------------------
// 4 — Club Admin cannot reach another club's players / player accounts
// ---------------------------------------------------------------------------
describe("club admin tenant isolation", () => {
  ...
  it("rejects listing another club's player accounts", async () => {
    await expect(listPlayerAccounts(clubAdminA, CLUB_B)).rejects.toBeInstanceOf(ForbiddenError);
  });
  ...
  it("allows the admin's own club", () => {
    ...
    expect(can(clubAdminA, "playerAccounts.manage", { clubId: CLUB_B })).toBe(false);
  });
});

// ---------------------------------------------------------------------------
// 5 — Multi-child player: one login resolves all linked children, scoped
// ---------------------------------------------------------------------------
describe("multi-child player scope", () => {
  it("each linked child is reachable; unrelated children are not", async () => {
    ...
    for (const kid of ["kid-1", "kid-2"]) {
      expect(can(playerMulti, "players.edit_limited_own_child", { clubId: CLUB_A, playerId: kid })).toBe(true);
    }
    // A child of another family is rejected at the service layer.
    await expect(
      updateOwnChild(playerMulti, "kid-of-another-family", { preferredName: "x" } as never),
    ).rejects.toBeInstanceOf(ForbiddenError);
  });
});

// ---------------------------------------------------------------------------
// 6 — Player edit accepts whitelisted fields, rejects everything else
// ---------------------------------------------------------------------------
describe("player edit whitelist (approved fields only)", () => {
  it("accepts the approved guardian fields", () => {
    const result = playerAccountUpdatePlayerSchema.safeParse({
      preferredName: "Lex",
  ...
  it("rejects coach/admin-owned fields (jersey number, status, position)", () => {
    expect(playerAccountUpdatePlayerSchema.safeParse({ jerseyNumber: "99" }).success).toBe(false);
    expect(playerAccountUpdatePlayerSchema.safeParse({ status: "ARCHIVED" }).success).toBe(false);
    expect(playerAccountUpdatePlayerSchema.safeParse({ primaryPosition: "GK" }).success).toBe(false);
  });
});

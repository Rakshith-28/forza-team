import { Bell, CalendarDays, Home, MessageCircle, User, Users } from "lucide-react";
...
/** Route links only — what the mobile bottom tab bar renders (bell excluded). */
export const PARENT_NAV_TABS = PARENT_NAV_ITEMS.filter(
  (item): item is ParentNavItem & { href: string } => item.kind === "link",
);
